                        ...
                        <div className="flex flex-col items-center gap-8 text-center">
                            {program.thumbnail && (
                                <div className="overflow-hidden rounded-lg shadow-xl">
                                    <img src={`/storage/${program.thumbnail}`} alt={program.title} className="h-36 object-cover md:h-56" />
                                </div>
                            )}
                            <div className="space-y-2">
                                <h1 className="text-3xl font-bold text-white md:text-4xl">{program.title}</h1>
                                <p className="text-blue-100 md:text-xl">Formulir Pengajuan Beasiswa</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="mx-auto w-full max-w-5xl px-4 py-8">
                    ...
                    <div className="mb-8 space-y-6 rounded-xl border-0 bg-gradient-to-br from-blue-50 to-indigo-50 p-2 shadow-md md:p-8 dark:from-blue-950/30 dark:to-indigo-950/30">
                        <div className="rounded-lg bg-white/60 p-3 text-center backdrop-blur-sm md:p-6 dark:bg-zinc-800/40">
                            <p className="font-semibold text-gray-900 md:text-lg dark:text-gray-100">
                                SkillGrow membuka Program Beasiswa bagi mahasiswa yang ingin meningkatkan kemampuan dan memperoleh sertifikasi profesional yang dibutuhkan di dunia kerja.
                            </p>
                        </div>
                        <div>
                        ...
                        <div className="to-primary rounded-lg bg-gradient-to-tl from-black p-4 text-white shadow-lg">
                            ...
                            <div className="space-y-1">
                                <p className="text-sm">📧 <span className="font-medium">skilgrow@gmail.com</span></p>
                                <p className="text-sm">💬 <span className="font-medium">+6285167541152</span></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="rounded-xl border-0 bg-white shadow-lg dark:bg-zinc-800">
                        <div className="to-primary rounded-t-xl border-b border-gray-200 bg-gradient-to-tl from-black px-6 py-6 dark:border-zinc-700">
                            <h2 className="font-bold text-white md:text-2xl">📝 Formulir Pendaftaran Beasiswa</h2>
                                ...
                                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                                    ...
                                    <div className="space-y-4 rounded-lg border-t border-gray-200 bg-gradient-to-r from-orange-50 to-amber-50 p-4 pt-5 md:p-5 dark:border-zinc-700 dark:from-orange-950/20 dark:to-amber-950/20">
                                        ...
                                        <FormField control={form.control} name="instagram_follow_photo" render={({ field: { onChange, value, ...field } }) => {
                                            void value; return (
                                                <FormItem><FormLabel className="flex items-center gap-2"><span>📱</span> Screenshot Follow Instagram @skillgrow.id *</FormLabel><FormControl><div className="space-y-3">
                                        ...
                                        <FormField control={form.control} name="tiktok_follow_photo" render={({ field: { onChange, value, ...field } }) => {
                                            void value; return (
                                                <FormItem><FormLabel className="flex items-center gap-2"><span>📱</span> Screenshot Follow TikTok @skillgrow.id *</FormLabel><FormControl><div className="space-y-3">
                                        ...
                                        <FormField control={form.control} name="instagram_story_photo" render={({ field: { onChange, value, ...field } }) => {
                                            void value; return (
                                                <FormItem><FormLabel className="flex items-center gap-2"><span>📲</span> Screenshot Share Postingan ke Story Instagram *</FormLabel><FormControl><div className="space-y-3">
                                                    {igStoryPreview && <div className="relative overflow-hidden rounded-lg border-2 border-fuchsia-300 bg-gradient-to-br from-fuchsia-50 to-pink-50 p-2 dark:from-fuchsia-950/30 dark:to-pink-950/30"><img src={igStoryPreview} alt="IG Story Preview" className="h-40 w-full rounded object-contain" /><div className="absolute top-2 right-2 rounded-full bg-fuchsia-500 px-2 py-1 text-xs font-semibold text-white">✓ Terpilih</div></div>}
                                                    <Input type="file" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) { onChange(file); handleFilePreview(e, setIgStoryPreview); } }} className="cursor-pointer border-2 border-dashed border-gray-300 py-6 hover:border-fuchsia-400 dark:border-zinc-600" {...field} />
                                                </div></FormControl><FormDescription className="text-xs">📸 Format: JPG, PNG, WebP (Maks 5MB)</FormDescription><FormMessage /></FormItem>
                                            );
                                        }} />
                                    </div>
